...
instrs = text.splitlines()

def parse_line(l):
	# reg name, delta, condition as string
	toks = l.split(maxsplit=4)
	return toks[0], int(toks[2]) * (1 if toks[1] == "inc" else -1), toks[4]

regs = dict((parse_line(l)[0], 0) for l in instrs)
highest = 0

def exec_line(l):
	reg, delta, condition = parse_line(l)
	exec("if {}:\n  {} += {}".format(condition, reg, delta), globals(), regs)

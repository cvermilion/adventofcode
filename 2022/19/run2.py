...
max_geodes = []
for (ii, bp) in enumerate(blueprints[:0]):
	id = ii+1
	print()
	print("BP", id, "******")
	print()
	...
	state_sets[t].add(state_hash(s))
	
	max_geo = 0
	tg, tb, tc = constraints(24, 0, bp)
	...
	while not states.empty():
		...
		t = -negt + 1
		if i % 100000 == 0:
			print("checking {} ({}) {}".format(i, max_geo, state_str(s)))
		if t == 24:
			s = incr(s) # final harvest
			if s.geode > max_geo:
				max_geo = s.geode
				tg, tb, tc = constraints(24, max_geo, bp)
			...
			if not h in checked:
				...
				for h2 in checked:
					...
					if better_hash(h2, h):
						improved.add(h2)
				if improved:
					checked.difference_update(improved)
				if not ignore:
				"""
	...
	max_geodes.append(id*max_geo)

print(sum(max_geodes))


# part 2

def constraints2(max_geo, bp):
	tg = math.floor(32.5 - .5 * math.sqrt(1+8*max_geo))
	tb = math.floor(tg - .5 - .5 * math.sqrt(1+8*bp.geo_obs_cost))
	tc = math.floor(tb - .5 - .5 * math.sqrt(1+8*bp.obs_clay_cost))
	return tg, tb, tc
...
max_geodes = []
for (id, bp) in enumerate(blueprints[:0]):
	print()
	print("BP", id, "******")
	print()
	...
	t,s = 0, start
	states.put((t, score2(s, bp), s))
	state_sets = dict((i,set()) for i in range(33))
	state_sets[t].add(s)
	
	max_geo = 0
	tg, tb, tc = constraints2(0, bp)
	...
	while not states.empty():
		...
		if t == 32:
			...
			if s.geode > max_geo:
				max_geo = s.geode
				tg, tb, tc = constraints2(max_geo, bp)
				print("new best:", state_str(s), tg,tb,tc)
			continue
			
		# check some cases where we can rule this state out
		# todo: can do better here, eg, at tg+2 one geode bot is not enough
		if s.geo_bot == 0 and t > tg:
			continue
		if s.obs_bot == 0 and t > tb:
			continue
		...
		for n in nn:
			if not n in checked:
				states.put((-t, score2(n, bp), n))
				checked.add(n)
	
	print(max_geo)
	t3 = time()
	print("full time", t3-t1)
	max_geodes.append(max_geo)

#print(max_geodes)

def max_geo(tmax, bp, approx=False):
	if tmax <= 1:
		return new_state()
	...
	best = new_state()
	if True:
		best_prev = max_geo(tmax-1, bp, True)
	...
	while not states.empty():
		i += 1
		#if i > 1000000 and approx:
		#	print("guessing", state_str(best))
		#	return best
		negt, _, s = states.get()
		t = -negt + 1
		if i % 100000 == 0:
...
for (id, bp) in enumerate(blueprints):
	...
	max_geodes.append(max_geo(32, bp))
	
print(max_geodes)
gg = [s.geode for s in max_geodes]
print(functools.reduce(operator.mul, gg))

text="""fjkfpm (69) -> kohxzh, liwvq, eqkio, xvoyybs
dsiixv (52)
fhimhm (66)
mdlubuq (73)
ulobwyb (41)
...
def parse_line(l):
	...
	children = [s.strip(',') for s in toks[3:]]
	return name, weight, children

progs = dict((n, (w, c)) for (n, w, c) in map(parse_line, text.splitlines()))

parents = {}
for name, (w, children) in progs.items():
	for c in children:
		parents[c] = name

root = [n for n in progs.keys() if n not in parents][0]
print(root)

def weight(name):
	w, children = progs[name]
	return w + sum(map(weight, children))

print(weight(root))

def children_equal(name):
	w, ch = progs[name]
	return len(set(map(weight, ch))) < 2

print(children_equal(root))

def problem_child(name):
	ch_weights = [(n, weight(n)) for n in progs[name][1]]
	print(ch_weights)
	corr_weight = [w for (n, w) in ch_weights if len([ww for (n, ww) in ch_weights if w == ww]) > 1][0]
	oddball = [n for (n, w) in ch_weights if w != corr_weight][0]
	return oddball, corr_weight
	
def bad_node(name):
	oddball, corr_weight = problem_child(name)
	if children_equal(oddball):
		return oddball, corr_weight
	return bad_node(oddball)

print(problem_child(root))
print(bad_node(root))

node, corr_total = bad_node(root)
print("total should be {}, node weight is {}, node total is {}".format(corr_total, progs[node][0], weight(node)))
